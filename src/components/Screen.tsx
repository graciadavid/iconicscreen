...
export function Screen({ style, fixed = true }: Props) {
  const { current } = useUploads()
  if (style.width === 0) return null
  return (
    <div style={{position:fixed?'fixed':'absolute',top:style.top,left:style.left,width:style.width,height:style.height,zIndex:3,overflow:'hidden',background:'#000'}}>
